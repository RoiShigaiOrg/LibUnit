...
void	display_header(t_test_group *test_group)
{
	...
	if (test_group->skipped_tests)
		ft_printf(" %s(skipped %i tests)%s", YELLOW, test_group->skipped_tests, RESET);
	ft_printf("\n\n");
}
...
void	display_footer(t_test_group *test_group, int success)
{
	...
	if (success == test_group->run_tests)
		ft_printf("%s%s tests success (%i / %i)%s", GREEN, test_group->function_name, success, test_group->run_tests, RESET);
	else
		ft_printf("%s%s tests failed (%i / %i)%s", RED, test_group->function_name, success, test_group->run_tests, RESET);
	ft_printf("\n\n");
}
...
int	display_result(t_test_group *test_group, t_unit_test *test, int status)
{
	...
	if (status_str == NULL)
		return (0);
	if (status == 0)
		ft_printf("%s: %s%s %s%s\n", test_group->function_name, GREEN, test->name, status_str, RESET);
	else
		ft_printf("%s: %s%s %s%s\n", test_group->function_name, RED, test->name, status_str, RESET);
	if (status_str != NULL)
		free(status_str);
	return (1);
}
